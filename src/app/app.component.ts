...
import {
  ...
  retry, tap, takeUntil, share,
  combineLatestWith,
  debounce, debounceTime, throttleTime, auditTime,
  take, withLatestFrom, startWith, scan, takeWhile,
  windowTime, sampleTime, toArray, window, bufferTime,
...
export class AppComponent {
  ...
  share() {
    ...
    const example = source.pipe(
      tap(v => console.log('Generated: ', v)),
    );

    const subscribe = example.subscribe(
      val => console.log('Not shared 1:', val));
    const subscribeTwo = example.subscribe(
      val => console.log('Not shared 2:', val));

    // share observable among subscribers
    const sharedExample = example.pipe(share());
    const subscribeThree = sharedExample.subscribe(
      val => console.log('Shared 1:', val));
    const subscribeFour = sharedExample.subscribe(
      val => console.log('Shared 2:', val));
  }
